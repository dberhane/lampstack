...
if Vagrant::Util::Platform.windows?
    ...
    if File.exists?(File.join(Dir.home, ".ssh", "bmj_rsa"))
       # Read local machine's GitHub SSH Key (~/.ssh/id_rsa)
        ...
        config.vm.provision :shell, :inline => "echo 'Windows-specific: Copying local Private GitHub SSH Key to VM for provisioning...' && mkdir -p /home/vagrant/.ssh && echo '#{private_ssh_key}' > /home/vagrant/.ssh/id_rsa && chmod 600 /home/vagrant/.ssh/id_rsa && chown vagrant /home/vagrant/.ssh/id_rsa"
    else
